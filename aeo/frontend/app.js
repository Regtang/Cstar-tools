...
let TOKEN = localStorage.getItem('cstar_token') || localStorage.getItem('aeo_token') || '';
function saveToken(t){TOKEN=t;localStorage.setItem('cstar_token',t);localStorage.setItem('aeo_token',t);}
let ME = null;
const CACHE = {};                  // 实体数据缓存
...
function setAuthMode(m){
  ...
  show('liToggleRow', m==='login');   // 通行证入口（注册已迁出本平台）
  show('liForgotRow', true);
  show('liHint', m==='login');
  $('#liUserLabel').textContent=(m==='forgot')?'用户名或邮箱':'用户名';
  $('#liPassLabel').textContent=(m==='reset')?'新密码（至少6位）':'密码';
  $('#liForgot').textContent=(m==='login')?'忘记密码？':'← 返回登录';
  $('#liErr').textContent='';$('#liErr').style.color='';
}
function afterAuth(){
  // AEO 平台仅限内部员工；外部客户登录态有效，但引导回工具平台首页
  if(ME && ME.isCustomer){ location.replace('/'); return; }
  enterApp();
}
...
function doLogout(){TOKEN='';localStorage.removeItem('cstar_token');localStorage.removeItem('aeo_token');ME=null;
...
/* 注册已迁至喜事达通行证（/account.html）；liToggle 现为普通链接 */
$('#liForgot').onclick=()=>setAuthMode(_authMode==='login'?'forgot':'login');
...
(async function init(){
  ...
  if(TOKEN){try{
    ME=await api('/me');
    if(ME.isCustomer){ location.replace('/'); return; }  // 外部客户不进合规平台，回工具平台首页
    enterApp();
  }catch(e){doLogout();}}
})();
