"""初始化数据：默认账号 + 海关高级认证 32 项标准框架（真实条款）。
...
STANDARD_FRAME = [
    ...
    ("2.2", "财务状况", "资产负债率", "无连续5年资产负债率超过95%情形", "财务部"),
    ("2.3", "财务状况", "偿付能力", "具备履行海关相关义务的财务能力", "财务部"),
    ("2.4", "财务状况", "依法纳税", "依法纳税，无重大欠税", "财务部"),
    ("3.1", "守法规范", "无走私违规记录", "近2年无走私犯罪、走私行为", "关务部/法务"),
    ("3.2", "守法规范", "报关差错率", "年度进出口报关差错率低于规定标准", "关务部"),
    ("3.3", "守法规范", "违规处置闭环", "对违规行为及时纠正并防止再发", "关务部"),
    ("3.4", "守法规范", "四类人员守法", "法定代表人、财务负责人等近1年无违法违规", "人力资源部"),
...
DEFAULT_TOOLS = [
    ...
    dict(slug="export-customs", name="出口海关研判助手", category="报关",
...
def run():
    ...
    try:
        ...
        if db.query(models.Standard).count() == 0:
