...
description: 喜事达工具平台（bot.cstar.com）前端统一标准 v2.1。当你要为喜事达/Cstar 开发、设计、改造或上线任何内部小工具、网页应用、计算器、表单或仪表板时，务必使用本技能，以保证配色、风格、交互、基本功能与平台一致——即使用户没有明说"按标准来"。涵盖统一配色与字体（装箱软件风格：深蓝页头+玫红主操作色+青色辅助）、cstar-ui.css 共享样式、弹窗嵌入模式、四项功能基线（示例数据/输入校验/导出打印/响应式+数据存储三选一）以及部署前自检清单与提交方式。Use this skill whenever building or restyling a tool for Cstar / 喜事达 / bot.cstar.com.
---
...
（v2.1 变更：数据存储从"一律本机暂存"改为**按工具实际需要三选一**——不需保存 / 本机暂存 / 云端保存；新增云端台账接口 `Cstar.cloud(tool)`；提交工具时需声明存储方式，平台自检按声明评判，纯计算/云端工具不再因缺本机暂存被扣分。）
...
- **历史台账（本机）`Cstar.records(key)`**：`add` / `all` / `get` / `update` / `remove` / `clear`；`search({text,fields,from,to,dateField})` 按关键词与日期范围筛选；`exportCSV(file,cols)` / `exportExcel(file,cols)` 一键导出台账。存浏览器本机（localStorage），刷新不丢、仅本机、不跨设备。
- **云端台账 `Cstar.cloud(tool)`（云端保存用）**：和 `records` 同一套思路，但存到平台数据库（`/api/store`），**跨设备、同组织多人共享、需登录平台**。方法均返回 Promise：`all()` / `add(data,scope)` / `get(id)` / `update(id,data,scope)` / `remove(id)`；`scope` 取 `'org'`（同组织共享，默认）或 `'private'`（仅本人）。例：`const db = Cstar.cloud('mytool'); await db.add({客户:'A'}); const list = await db.all();`。数据按组织强制隔离，外部客户永远读不到别家数据。
...
**数据存储按工具实际需要三选一（不再一刀切要求本机暂存）：**

1. **不需保存**：纯计算 / 查询类工具，结果用完即走，不必接入任何存储。
2. **本机暂存**：用 `Cstar.store.bindForm` 自动暂存表单、`Cstar.records` 做本机台账（localStorage）。适合个人草稿、半路刷新不丢，但仅本机、不跨设备。
3. **云端保存**：用 `Cstar.cloud(tool)` 存到平台（`/api/store`）。适合需要跨设备、同组织多人共享、留档的台账。需登录。

提交工具时，bot.cstar.com 的提交页会要求**声明本工具用哪一种**；平台自检按你的声明评判——选「不需保存」或「云端保存」的工具，不会再因为缺少本机暂存而被提示缺项。`assets/cstar-tool.js` 是其副本。
...
4. **响应式 + 数据存储（三选一）** —— 手机可用（cstar-ui.css 已含断点）；数据存储按工具需要在「不需保存 / 本机暂存 / 云端保存」中选一种并在提交时声明（见第〇节）。原因：手机现场要能用；需要留存的数据不该半路刷新或断网就丢，而纯计算工具不必强行接存储。
...
### 第 2 步：功能测试 + 基本功能基线
- [ ] 四项基线齐全：示例数据 / 输入校验 / 导出打印 / 响应式+数据存储（已按工具选定「不需保存/本机暂存/云端保存」之一，提交时如实声明）
- [ ] 核心流程、边界值、异常输入都测过一遍
