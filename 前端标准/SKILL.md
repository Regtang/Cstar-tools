...
description: 喜事达工具平台（bot.regs.com）前端统一标准 v2.0。当你要为喜事达/Cstar 开发、设计、改造或上线任何内部小工具、网页应用、计算器、表单或仪表板时，务必使用本技能，以保证配色、风格、交互、基本功能与平台一致——即使用户没有明说"按标准来"。涵盖统一配色与字体（装箱软件风格：深蓝页头+玫红主操作色+青色辅助）、cstar-ui.css 共享样式、弹窗嵌入模式、四项功能基线（示例数据/输入校验/导出打印/响应式+本地保存）以及部署前自检清单与提交方式。Use this skill whenever building or restyling a tool for Cstar / 喜事达 / bot.regs.com.
---

# 喜事达工具平台 · 前端统一标准 v2.0

平台地址 **bot.regs.com** 上挂着多个由不同同事开发的小工具（AEO 合规、装箱、危化判定、出口研判、HS 速查、单证生成等）。为了让客户和内部看到统一、专业的体验，所有工具的前端必须遵循同一套标准。本技能告诉你怎么做。
...
5. 通过 bot.regs.com 首页「上传工具 / 提交工具更新」入口提交，由平台维护者审核部署。
...
- **署名（必备）`Cstar.stamp({tool, version, author})`**：在页脚显示「工具名 + 版本号 + 作者姓名」，并供生成的单据/导出引用 `Cstar.stampLine()`。**每个工具都必须声明作者姓名与版本号。**
...
不要自己从零写配色和组件——直接引入平台维护的共享样式表，自动获得统一配色、字体与组件。`assets/cstar-ui.css` 是其副本，线上同源地址为 `https://bot.regs.com/cstar-ui.css`。
...
把工具打包成单个 `.zip`，到 bot.regs.com 首页底部「For Developers」模块：
- 新工具点 **「⬆ 上传新工具」**
